...
FROM
    'backend/db/cleaned/food.csv' WITH (FORMAT csv, HEADER);

COPY phaero_food.nutrient (id, name, unit_name)
FROM
    'backend/db/cleaned/nutrient.csv' WITH (FORMAT csv, HEADER);

COPY phaero_food.food_nutrient (id, fdc_id, nutrient_id, amount)
FROM
    'backend/db/cleaned/food_nutrient.csv' WITH (FORMAT csv, HEADER);
...
FROM
    'backend/db/cleaned/branded_food.csv' WITH (FORMAT csv, HEADER);

COPY phaero_food.fndss_ingredient_nutrient_value (ingredient_description, nutrient_id, amount)
FROM
    'backend/db/cleaned/fndss_ingredient_nutrient_value.csv' WITH (FORMAT csv, HEADER);

COPY phaero_food.food_portion (fdc_id,amount,gram_weight,portion_description)
FROM
    'backend/db/cleaned/food_portion.csv' WITH (FORMAT csv, HEADER);

COPY phaero_food.input_food (fdc_id, amount, sr_description, gram_weight)
FROM
    'backend/db/cleaned/input_food.csv' WITH (FORMAT csv, HEADER);

COPY phaero_food.survey_fndds_food (fdc_id, wweia_category_code)
FROM
    'backend/db/cleaned/survey_fndds_food.csv' WITH (FORMAT csv, HEADER);
COPY phaero_food.foundation_food (fdc_id, description)
FROM
    'backend/db/cleaned/foundation_food.csv' WITH (FORMAT csv, HEADER);
COPY phaero_food.german_foundation_food (fdc_id, description)
FROM
    'backend/db/cleaned/german_foundation_food.csv' WITH (FORMAT csv, HEADER);
COPY phaero_food.userrecommendations (name, unit_name, default_amount)
FROM
    'backend/db/cleaned/userrecommendations.csv' WITH (FORMAT csv, HEADER);
COPY phaero_food.german_food (fdc_id, description)
FROM
    'backend/db/cleaned/german_food.csv' WITH (FORMAT csv, HEADER);
